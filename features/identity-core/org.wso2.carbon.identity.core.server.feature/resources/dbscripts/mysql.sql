...
-- Create IDN_REG_FLOW table
CREATE TABLE IDN_REG_FLOW (
    ID VARCHAR(50) PRIMARY KEY,
    TENANT_ID VARCHAR(50),
    FLOW_NAME VARCHAR(255),
    FLOW_JSON BLOB  -- Storing the flow definition as binary data or large text
);

-- Create IDN_REG_NODE table with auto-incremental ID
CREATE TABLE IDN_REG_NODE (
    ID INT PRIMARY KEY AUTO_INCREMENT,  -- Unique auto-incremented ID
    NODE_ID VARCHAR(50),
    FLOW_ID VARCHAR(50),
    NODE_TYPE VARCHAR(100),
    IS_FIRST_NODE BOOLEAN DEFAULT FALSE,  -- Indicates if this is the first node of the flow
    UNIQUE (NODE_ID, FLOW_ID),  -- Ensures NODE_ID is unique per flow
    FOREIGN KEY (FLOW_ID) REFERENCES IDN_REG_FLOW(ID)
);

-- Create IDN_REG_PAGE table
CREATE TABLE IDN_REG_PAGE (
    ID INT PRIMARY KEY AUTO_INCREMENT,  -- Auto-incremental primary key
    IDN_REG_NODE_ID INT,  -- References auto-incremented ID of IDN_REG_NODE
    PAGE_CONTENT BLOB,
    FOREIGN KEY (IDN_REG_NODE_ID) REFERENCES IDN_REG_NODE(ID) ON DELETE CASCADE
);

-- Create IDN_REG_NODE_PROPERTIES table
CREATE TABLE IDN_REG_NODE_PROPERTIES (
